...
const debug = require("debug")("gamebot:api-client");
...
export class CacheGraphQlQueryExecutor extends GraphQlQueryExecutor {
  ...
  constructor(
    request: IGraphQlQueryExecutorRequest,
    cacheOptions: CacheGraphQlQueryExecutorOptions
  ) {
    super(request);
    for (let key of Object.keys(cacheOptions)) {
      this.storage[key] = new LRU({
        max: cacheOptions[key].max,
        maxAge: cacheOptions[key].ttl
      });
    }
  }
