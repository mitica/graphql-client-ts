...
async function run() {
    ...
    const typesGenerator = new TypesGenerator(schemaData, join(output, typesFilename + '.ts'));
    const queryApiFilename = 'query-api';
    const queryApiGenerator = new ApiGenerator(schemaData, join(output, queryApiFilename + '.ts'), 'query', typesFilename);
    const mutationApiFilename = 'mutation-api';
    const mutationApiGenerator = new ApiGenerator(schemaData, join(output, mutationApiFilename + '.ts'), 'mutation', typesFilename);
    const clientFilename = 'graphql-client';
    const clientGenerator = new ClientGenerator(join(output, clientFilename + '.ts'), queryApiFilename, mutationApiFilename);
