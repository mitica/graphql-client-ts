...
export class SchemaData {
  ...
  getTypeByName(name: string): TypeData {
    return this.schema.data.__schema.types.find(
      (type: TypeData) => type.name === name
    );
  }
  ...
  isOfType(type: TypeData, types: string[]) {
    ...
    return (
      name.indexOf("__") !== 0 &&
      invalidNames.indexOf(name) < 0 &&
      types.indexOf(kind) > -1
    );
  }
  ...
  static async create(url: string, headers?: any) {
    ...
    headers["origin"] =
      headers["origin"] || `${parsedUrl.protocol}//${parsedUrl.hostname}`;

    // console.log('headers', headers)

    const response = await got(url, {
      headers,
      body: { query: schemaQuery, variables: null },
      method: "POST",
      json: true
    });
    if (response.statusCode >= 400) {
      const data = await response.body;
      throw new Error(`Bad response(${response.statusCode}) from server: ${data}`);
    }
    const schema = await response.body;
...
export function generateTypeScriptType(
  items: { name: string; type: string; required: boolean }[]
): string {
  return `{ ${items
    .map(item => `${item.name}${item.required ? "" : "?"}: ${item.type}`)
    .join(", ")} } `;
}
