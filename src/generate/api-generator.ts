...
import { ActionType, SchemaData, TypeData, GeneratedInfo, typeIsRequired, typeIsList, getTypeName, getJsTypeName, generateTypeScriptType } from "./schema";
import { startWithUpperCase, uniq } from "../utils";

export class ApiGenerator extends FileGenerator {
    constructor(private schemaData: SchemaData, file: string, private type: ActionType, private apiTypesFilename: string) {
        super(file)
    }

    protected async getContent(): Promise<string> {
        const methods = this.type === 'mutation' ? this.generateMutationMethods() : this.generateQueryMethods();
...
function generateApi(action: ActionType, schemaData: SchemaData, apiTypesFilename: string) {
    const schema = schemaData.schema;
    const typeExists = !!schema.data.__schema[action + 'Type'];

    const type = typeExists ? schemaData.getTypeByName(schema.data.__schema[action + 'Type'].name) : { fields: [] } as any;
    ...
    const methods = type.fields.map((field: any) => {
        ...
        const resultIsObject = schemaData.typeIsObject(field.type) || typeIsList(field.type) && schemaData.typeIsObject(field.type.ofType.ofType);
        ...
        const argData = {
            name: 'args',
            type: generateTypeScriptType(field.args.map((arg: any) => {
...
function generateMethods(action: ActionType, type: TypeData): GeneratedInfo {
    const names = type ? type.fields.map(field => `    ${field.name} = "${field.name}"`) : [];
